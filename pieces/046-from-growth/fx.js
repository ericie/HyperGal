// From Growth — Eric Ishii Eckhardt, 2025.
//
  ...
  const $fx = {
    ...
    params(defs) {
      ...
      for (const def of defs) {
        ...
        // A parameter with no declared default was filled in by the platform
        // from the hash. Drawing it from the seeded stream, in definition
        // order, keeps that deterministic per hash.
        const base = def.default !== undefined ? def.default : randomValue(def, $fx.rand);
        const value = fromUrl !== undefined ? fromUrl : base;
    ...
    getParamDefinition: (id) => definitions.find((d) => d.id === id),
