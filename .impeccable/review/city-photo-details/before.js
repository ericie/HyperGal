(() => {
  ...
  function sash(x, y, w, h, ink, paper, panes = 2) {
    rect(x, y, w, h, ink);
    const frame = Math.max(Math.min(w, h) * 0.085, 0.4);
    for (let i = 0; i < panes; i++) {
      const pw = w / panes;
      rect(x + i * pw + frame, y + frame, Math.max(0, pw - frame * 1.7), Math.max(0, h - frame * 2), paper);
    }
  }
  function condenser(x, y, w, h, ink, paper) {
    rect(x, y, w, h, ink);
    rect(x + w * 0.075, y + h * 0.1, w * 0.85, h * 0.75, paper);
    const r = Math.min(w * 0.23, h * 0.29);
    ellipse(x + w * 0.34, y + h * 0.47, r, r, ink);
    ellipse(x + w * 0.34, y + h * 0.47, r * 0.57, r * 0.57, paper);
    for (let i = 0; i < 3; i++) rect(x + w * 0.69, y + h * (0.23 + i * 0.18), w * 0.15, h * 0.065, ink);
    rect(x + w * 0.15, y + h * 0.85, w * 0.1, h * 0.15, ink);
    rect(x + w * 0.75, y + h * 0.85, w * 0.1, h * 0.15, ink);
  }
  ...
  tokyo('roller-shop', 'Roller shutter + side passage', 'base', ({ w, h, ink, paper, variant }) => {
    ...
    const rows = 7;
    for (let i = 0; i < rows; i++) rect(w * 0.09, h * (0.28 + i * 0.087), w * 0.53, h * 0.026, paper);
  ...
  tokyo('tenant-floors', 'Stacked businesses + projecting signs', 'chamber', ({ w, h, ink, paper, variant }) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      sash(w * 0.07 + setback, y + fh * 0.2, w * 0.56 - setback, fh * 0.4, ink, paper, 2 + (i + variant) % 2);
  ...
  tokyo('balcony-stack', 'Shallow balconies + condensers', 'chamber', ({ w, h, ink, paper, variant }) => {
    const n = clamp(Math.round(h / w * 1.3), 2, 4), fh = h / n;
    for (let i = 0; i < n; i++) {
      const y = fh * i, shift = (i + variant) % 2 ? w * 0.06 : 0;
      sash(w * 0.11 + shift, y + fh * 0.08, w * 0.49, fh * 0.5, ink, paper, 2);
      condenser(w * 0.66, y + fh * 0.33, w * 0.23, fh * 0.2, ink, paper);
      rect(w * 0.06, y + fh * 0.74, w * 0.88, fh * 0.065, ink);
      rail(w * 0.07, y + fh * 0.55, w * 0.86, fh * 0.19, ink, 8);
      rect(w * 0.07, y + fh * 0.55, w * (i % 2 ? 0.24 : 0.36), fh * 0.19, ink);
      line(w * 0.97, y + fh * 0.09, w * 0.97, y + fh * 0.96, ink, Math.max(0.8, w * 0.012));
    }
  });
  tokyo('service-wall', 'Party wall + ducts and service windows', 'chamber', ({ w, h, ink, paper, variant }) => {
    const rows = clamp(Math.round(h / w * 1.2), 2, 5), fh = h / rows;
    rect(w * 0.71, h * 0.015, w * 0.09, h * 0.97, ink);
    rect(w * 0.735, h * 0.02, w * 0.026, h * 0.96, paper);
    for (let i = 0; i < rows; i++) {
      const y = i * fh;
      sash(w * 0.14, y + fh * 0.12, w * 0.24, fh * 0.35, ink, paper, 1);
      if ((i + variant) % 2 === 0) condenser(w * 0.15, y + fh * 0.59, w * 0.4, Math.min(fh * 0.21, w * 0.27), ink, paper);
      rect(w * 0.66, y + fh * 0.31, w * 0.19, fh * 0.018, ink);
      line(w * 0.05, y + fh * 0.93, w * 0.57, y + fh * 0.93, ink, Math.max(0.6, w * 0.008));
    }
  });
  ...
  function compose(w, h, value) {
    ...
    const towers = widths.map((ratio, index) => {
      ...
      let ids;
      if (hero) {
        ids = [pick(random, ['tenant-floors', 'setback-rooms']), featureBlocks.get(index), pick(random, ['tenant-floors', 'balcony-stack'])];
      } else if (narrow) {
