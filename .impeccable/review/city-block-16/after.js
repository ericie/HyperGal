(() => {
  ...
  tokyo('tenant-entry', 'Recessed tenant lobby + mailboxes', 'base', ({ w, h, ink, paper }) => {
    rect(0, 0, w, h, paper);
    rect(w*0.05,h*0.06,w*0.9,h*0.09,ink);
    rect(w*0.11,h*0.09,w*0.62,h*0.027,paper);
    rect(w*0.08,h*0.22,w*0.69,h*0.75,ink);
    rect(w*0.13,h*0.26,w*0.56,h*0.71,paper);
    // Door leaves terminate on the same threshold as their jambs.
    rect(w*0.18,h*0.31,w*0.45,h*0.63,ink);
    for (let i=0;i<2;i++) {
      rect(w*(0.2+i*0.215),h*0.34,w*0.185,h*0.43,paper);
      rect(w*(0.2+i*0.215),h*0.8,w*0.185,h*0.11,paper);
    }
    rect(w*0.375,h*0.64,w*0.013,h*0.13,ink);
    rect(w*0.43,h*0.64,w*0.013,h*0.13,ink);
    for(let i=0;i<4;i++) {
      rect(w*0.81,h*(0.38+i*0.08),w*0.12,h*0.065,ink);
      rect(w*0.825,h*(0.393+i*0.08),w*0.08,h*0.008,paper);
    }
    rect(w*0.82,h*0.24,w*0.07,h*0.085,ink);
    rect(w*0.84,h*0.255,w*0.03,h*0.026,paper);
    rect(w*0.05,h*0.97,w*0.9,h*0.03,ink);
  });
  ...
  function stairTowerLayout(w, h, variant) {
    const count = clamp(Math.round(h / w * 1.5), 2, 5);
    const top = h * 0.1, rise = h * 0.85 / count;
    return Array.from({ length: count }, (_, i) => {
      const reverse = (i + variant) % 2;
      return { from: [w * (reverse ? 0.16 : 0.65), top + i * rise],
        to: [w * (reverse ? 0.65 : 0.16), top + (i + 1) * rise] };
    });
  }
  function steelFlight(from, to, railHeight, ink, weight, steps = 10) {
    const [x, y] = from, dx = to[0] - x, dy = to[1] - y;
    const points = [[x,y]];
    for(let j=0;j<steps;j++) {
      points.push([x+dx*(j+1)/steps,y+dy*j/steps]);
      points.push([x+dx*(j+1)/steps,y+dy*(j+1)/steps]);
    }
    points.push([to[0],to[1]+weight*2],[x,y+weight*2]);
    polygon(points,ink);
    line(x,y-railHeight,to[0],to[1]-railHeight,ink,weight);
    for(let j=0;j<=6;j++) line(x+dx*j/6,y+dy*j/6,x+dx*j/6,y+dy*j/6-railHeight,ink,weight*0.65);
  }
  tokyo('external-stair', 'Continuous switchback stairs + shared landings', 'chamber', ({ w, h, ink, paper, variant }) => {
    const flights = stairTowerLayout(w,h,variant), rise = flights[0].to[1]-flights[0].from[1];
    const weight=Math.max(w*0.009,0.45), railHeight=rise*0.19;
    for(const x of [0.07,0.72]) rect(w*x,0,w*0.022,h,ink);
    for(const flight of flights) steelFlight(flight.from,flight.to,railHeight,ink,weight);
    // Each flight ends at the exact point where the next one begins.
    const landings=[flights[0].from,...flights.map(f=>f.to)];
    for(let i=0;i<landings.length;i++) {
      const [x,y]=landings[i], left=x<w*0.4;
      const start=left?w*0.045:w*0.63, end=left?w*0.185:w*0.96;
      rect(start,y,end-start,rise*0.045,ink);
      rail(start,y-railHeight,end-start,railHeight,ink,left?2:4);
      if(!left && i>0) boxDoor(w*0.79,y-rise*0.55,w*0.13,rise*0.55,ink,paper);
    }
    rect(w*0.04,h*0.985,w*0.92,h*0.015,ink);
  }, { narrow: false, hero: true });
  ...
  tokyo('water-tank', 'Panelled water tank + ladder and anchored frame', 'crown', ({ w, h, ink, paper }) => {
    const tx=w*0.23, tw=w*0.5, cap=Math.min(h*0.043,w*0.04), deck=h*0.94;
    rect(tx,h*0.14,tw,h*0.43,ink);
    ellipse(tx+tw/2,h*0.14,tw/2,cap,ink);
    ellipse(tx+tw/2,h*0.57,tw/2,cap,ink);
    // Pressed metal panels, horizontal seams and inspection hatch.
    for(const f of [0.25,0.39,0.51]) line(tx+w*0.025,h*f,tx+tw-w*0.025,h*f,paper,w*0.008);
    for(const f of [0.25,0.5,0.75]) line(tx+tw*f,h*0.17,tx+tw*f,h*0.54,paper,w*0.006);
    rect(tx+tw*0.37,h*0.055,tw*0.26,h*0.072,ink);
    rect(tx+tw*0.32,h*0.048,tw*0.36,h*0.018,ink);
    rect(tx-w*0.025,h*0.6,tw+w*0.05,h*0.035,ink);
    for(const f of [0.1,0.9]) {
      const x=tx+tw*f;
      rect(x-w*0.012,h*0.575,w*0.024,deck-h*0.575,ink);
      rect(x-w*0.043,deck-h*0.015,w*0.086,h*0.03,ink);
    }
    line(tx+tw*0.1,h*0.64,tx+tw*0.9,deck,ink,w*0.012);
    line(tx+tw*0.9,h*0.64,tx+tw*0.1,deck,ink,w*0.012);
    for(const x of [0.12,0.2]) line(w*x,h*0.14,w*x,deck,ink,w*0.01);
    for(let i=0;i<9;i++) line(w*0.12,h*(0.19+i*0.088),w*0.2,h*(0.19+i*0.088),ink,w*0.008);
    conduit([[tx+tw,h*0.47],[w*0.86,h*0.47],[w*0.89,h*0.5],[w*0.89,deck]],ink,paper,w*0.028);
    rect(0,deck,w,h-deck,ink);
  });
  ...
  tokyo('plant-room', 'Roof access house + grounded equipment', 'crown', ({ w, h, ink, paper, variant }) => {
    const deck=h*0.94, x=variant%2?w*0.08:w*0.5, cx=variant%2?w*0.62:w*0.07;
    rect(x,h*0.2,w*0.39,deck-h*0.2,ink);
    boxDoor(x+w*0.075,h*0.5,w*0.19,deck-h*0.5,paper,ink);
    rect(x-w*0.025,h*0.16,w*0.44,h*0.04,ink);
    const equipmentHeight=Math.min(h*0.31,w*0.18);
    condenser(cx,deck-equipmentHeight,w*0.27,equipmentHeight,ink,paper);
    conduit([[cx+w*0.25,deck-equipmentHeight*0.3],[cx+w*0.29,deck-equipmentHeight*0.3],[cx+w*0.29,deck]],ink,paper,w*0.014);
    // Every wall, door and equipment foot lands on this same slab.
    rect(0,deck,w,h-deck,ink);
    rail(w*0.02,h*0.76,w*0.96,deck-h*0.76,ink,9);
  });
  ...
  tokyo('porthole-bays', 'Paired round bays + open stair landings', 'chamber', ({ w, h, ink, paper, variant }) => {
    const n = clamp(Math.round(h / w * 1.5), 2, 5), fh = h / n;
    rect(w * 0.07, h * 0.02, w * 0.86, h * 0.96, ink);
    const r = Math.min(w * 0.14, fh * 0.34);
    for (let i = 0; i < n; i++) {
      const cy = (i + 0.49) * fh;
      for (let j = 0; j < 2; j++) {
        const cx = w * (0.28 + j * 0.43);
        ...
        if (j === 0) {
          ...
          rect(cx-r,cy-r,r*2,r*2,paper);
          rect(cx-r,cy-r,r*2,r*((i+variant)%2?0.35:0.65),ink);
          line(cx-r,cy+r*0.25,cx+r,cy+r*0.25,ink,r*0.055);
          line(cx,cy-r,cx,cy+r,ink,r*0.065);
        }
        ctx.restore();
      }
      line(w*0.08,(i+0.96)*fh,w*0.92,(i+0.96)*fh,paper,w*0.004);
    }
  }, { narrow: false });
  tokyo('conduit-wall', 'Tiled service wall + meter boxes and routed conduits', 'chamber', ({ w, h, ink, paper, variant }) => {
    ...
    // Clear every equipment pad before routing: later pads used to erase
    // the last segment of each wire just before it reached its meter box.
    rect(w*0.13,h*0.13,w*0.38,h*0.25,paper);
    sash(w*0.15,h*0.15,w*0.32,h*0.2,ink,paper,2,variant);
    const meters = Array.from({length:3}, (_,i) => ({x:w*(0.105+i*0.13),y:h*(i===1?0.43:0.4)}));
    for (const {x,y} of meters) rect(x-w*0.015,y-h*0.015,w*0.13,h*0.15,paper);
    rect(w*0.525,h*0.025,w*0.17,h*0.1,paper);
    rect(w*0.13,h*0.78,w*0.37,h*0.18,paper);
    rect(w*0.775,h*0.895,w*0.155,h*0.075,paper);
    ...
    for (let i=0;i<3;i++) {
      ...
      const meter=meters[i], mx=meter.x+w*0.065, bottom=meter.y+h*0.12;
      conduit([[px,h*0.105],[px,h*0.43],[px+w*0.035,h*0.47],[px+w*0.035,yy-h*0.025],[px+w*0.005,yy],[mx+w*0.03,yy],[mx,yy-h*0.035],[mx,bottom]],ink,paper,w*0.012);
      rect(mx-w*0.014,bottom,w*0.028,h*0.018,ink);
      rect(px-w*0.012,h*0.105,w*0.024,h*0.018,ink);
    }
    rect(w*0.54,h*0.04,w*0.14,h*0.065,ink);
    rect(w*0.55,h*0.047,w*0.12,h*0.05,paper);
    for(const {x,y} of meters) {
      rect(x,y,w*0.1,h*0.12,ink);
      rect(x+w*0.009,y+h*0.007,w*0.079,h*0.104,paper);
      rect(x+w*0.071,y+h*0.054,w*0.009,h*0.025,ink);
    }
    const outlet=condenser(w*0.16,h*0.79,w*0.3,Math.min(h*0.17,w*0.2),ink,paper);
    conduit([outlet,[w*0.53,outlet[1]],[w*0.58,h*0.93],[w*0.79,h*0.93]],ink,paper,w*0.019);
    rect(w*0.775,h*0.919,w*0.025,h*0.022,ink);
    rect(w*0.79,h*0.905,w*0.125,h*0.05,ink);
    rect(w*0.8,h*0.912,w*0.105,h*0.036,paper);
  }, { narrow: false });
  ...
  tokyo('commercial-front', 'Stacked commercial facade + sign tower', 'chamber', ({ w, h, ink, paper, variant }) => {
    const floors=clamp(Math.round(h/w*1.6),2,4), fh=h/floors;
    const face=w*0.69, edge=w*0.94;
    // Continuous columns and floor beams hold the glazing and tenant signs.
    rect(w*0.045,0,w*0.025,h,ink);
    rect(face,0,w*0.025,h,ink);
    rect(w*0.92,0,w*0.02,h,ink);
    for(let i=0;i<floors;i++) {
      const y=i*fh;
      rect(w*0.055,y+fh*0.015,edge-w*0.055,fh*0.035,ink);
      sash(w*0.09,y+fh*0.12,w*0.565,fh*0.47,ink,paper,3,i+variant);
      // A transom and paired vertical facade fins unify unequal tenants.
      line(w*0.09,y+fh*0.23,w*0.655,y+fh*0.23,ink,w*0.008);
      for(const x of [0.285,0.48]) rect(w*x,y+fh*0.05,w*0.012,fh*0.58,ink);
      const reverse=(i+variant)%2, labelInk=reverse?paper:ink, labelPaper=reverse?ink:paper;
      rect(w*0.075,y+fh*0.64,w*0.6,fh*0.26,ink);
      rect(w*0.087,y+fh*0.655,w*0.576,fh*0.225,paper);
      rect(w*0.105,y+fh*0.682,w*(i%2?0.37:0.53),fh*0.17,labelInk);
      rect(w*0.12,y+fh*0.704,w*(i%2?0.335:0.495),fh*0.125,labelPaper);
      rect(w*0.055,y+fh*0.955,edge-w*0.055,fh*0.045,ink);
      // The service bay is recessed behind the attached vertical sign tower.
      louvers(w*0.745,y+fh*0.13,w*0.12,fh*0.22,ink,paper);
      line(w*0.89,y+fh*0.05,w*0.89,y+fh*0.955,ink,w*0.009);
    }
    const sx=w*0.705, sy=h*0.31, sw=w*0.175, sh=h*0.52;
    for(const f of [0.08,0.88]) rect(face,sy+sh*f,w*0.035,h*0.014,ink);
    rect(sx,sy,sw,sh,ink);
    rect(sx+sw*0.075,sy+sh*0.018,sw*0.82,sh*0.964,paper);
    const signs=3;
    for(let i=0;i<signs;i++) {
      const y=sy+sh*i/signs;
      if(i%2===0) rect(sx+sw*0.14,y+sh*0.025,sw*0.68,sh/signs-sh*0.05,ink);
      if(i>0) line(sx,y,sx+sw,y,ink,w*0.008);
    }
  }, { narrow: false, hero: true });
  tokyo('stepped-terraces', 'Stepped apartments + continuous balcony slabs', 'chamber', ({ w, h, ink, paper, variant }) => {
    const floors=clamp(Math.round(h/w*1.6),3,5), fh=h/floors;
    ctx.save(); if(variant%2) {ctx.translate(w,0);ctx.scale(-1,1);}
    rect(w*0.91,0,w*0.02,h,ink);
    // Setbacks change at floor boundaries, leaving a usable terrace each time.
    for(let i=0;i<floors;i++) {
      const left=w*(0.35-i*0.28/(floors-1)), y=i*fh, right=w*0.93;
      rect(left,y+fh*0.04,right-left,fh*0.94,ink);
      rect(left+w*0.025,y+fh*0.09,right-left-w*0.05,fh*0.68,paper);
      const ww=(right-left)*0.51;
      sash(left+w*0.045,y+fh*0.14,ww,fh*0.46,ink,paper,2,i+variant);
      boxDoor(right-w*0.15,y+fh*0.14,w*0.1,fh*0.61,ink,paper);
      rail(left+w*0.022,y+fh*0.56,right-left-w*0.044,fh*0.21,ink,10);
      rect(left,y+fh*0.83,right-left,fh*0.08,paper);
      if(i>0) {
        const previousLeft=w*(0.35-(i-1)*0.28/(floors-1));
        rail(left,y-fh*0.09,previousLeft-left,fh*0.13,ink,2);
      }
      line(right-w*0.012,y+fh*0.04,right-w*0.012,y+fh*0.98,paper,w*0.006);
    }
    rect(w*0.065,h*0.985,w*0.87,h*0.015,ink);
    ctx.restore();
  }, { narrow: false, hero: true });
  tokyo('duct-deck', 'Rooftop duct bank + screened service deck', 'crown', ({ w, h, ink, paper }) => {
    const deck=h*0.94;
    // A continuous plant enclosure carries three bent exhaust risers.
    rect(w*0.13,h*0.56,w*0.74,deck-h*0.56,ink);
    louvers(w*0.17,h*0.62,w*0.3,h*0.25,paper,ink);
    rect(w*0.53,h*0.62,w*0.27,h*0.25,paper);
    for(let i=0;i<3;i++) {
      const x=w*(0.24+i*0.22), top=h*(0.12+i*0.09), pipe=w*0.07;
      rect(x,top,pipe,h*0.58-top,ink);
      rect(x-pipe*0.6,top-pipe*0.3,pipe*1.6,pipe,ink);
      rect(x-pipe*0.61,top-pipe*0.1,pipe*0.35,pipe*0.55,paper);
      line(x+pipe*0.35,top+pipe,x+pipe*0.35,h*0.54,paper,w*0.008);
      for(const y of [0.39,0.5]) rect(x-pipe*0.12,h*y,pipe*1.24,h*0.012,ink);
    }
    for(const x of [0.055,0.115]) line(w*x,h*0.47,w*x,deck,ink,w*0.009);
    for(let i=0;i<6;i++) line(w*0.055,h*(0.51+i*0.079),w*0.115,h*(0.51+i*0.079),ink,w*0.007);
    rect(0,deck,w,h-deck,ink);
    rail(w*0.02,h*0.73,w*0.96,deck-h*0.73,ink,10);
  }, { narrow: false });
  ...
  function compose(w, h, value) {
    ...
    const towers = widths.map((ratio, index) => {
      ...
      const tower = { x, w: bw, top, baseTop, baseLine, paper, ink, index, modules: [],
        ...
        profile: narrow ? 'straight' : featureBlocks.get(index) === 'stepped-terraces' ? 'terrace' : pick(random, ['terrace', 'straight', 'straight']),
