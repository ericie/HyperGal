(() => {
  ...
  tokyo('public-concourse', 'Public stair + glazed upper concourse', 'chamber', ({ w, h, ink, paper, variant }) => {
    const upper=h*0.33, floor=h*0.95;
    rect(w*0.04,h*0.035,w*0.92,h*0.035,ink);
    for(const x of [0.04,0.92]) rect(w*x,h*0.07,w*0.04,floor-h*0.07,ink);
    sash(w*0.13,h*0.09,w*0.5,h*0.18,ink,paper,4,variant);
    boxDoor(w*0.72,h*0.08,w*0.14,upper-h*0.08,ink,paper);
    rect(w*0.08,upper,w*0.84,h*0.025,ink);
    rail(w*0.1,upper-h*0.075,w*0.55,h*0.075,ink,8);
    // A straight steel flight runs from the ground slab to the upper landing.
    const top=[w*0.77,upper+h*0.025], bottom=[w*0.2,floor];
    steelFlight(top,bottom,h*0.065,ink,w*0.009,15);
    rect(w*0.075,floor,w*0.21,h*0.025,ink);
    rect(w*0.745,upper+h*0.025,w*0.17,h*0.025,ink);
    // Store/service opening under the stair, within its structural span.
    boxDoor(w*0.71,h*0.71,w*0.15,floor-h*0.71,ink,paper);
    louvers(w*0.48,h*0.8,w*0.14,h*0.11,ink,paper);
    rect(w*0.04,floor,w*0.92,h-floor,ink);
  }, { narrow: false, hero: true });
  ...
  const catalogSlots = ['shop-lobby','roller-shop','tenant-entry','tenant-floors',
    ...
    'public-concourse','stepped-terraces','duct-deck'];
  ...
  function compose(w, h, value) {
    ...
    const concoursePosition = Math.floor(broad.length / 2);
    const featureBlocks = new Map([
      [broad[concoursePosition], 'public-concourse'],
    ...
    const towers = widths.map((ratio, index) => {
      ...
      const top = h * (featureBlocks.get(index) === 'public-concourse' ? 0.19 : 0.17 + random() * 0.24);
      ...
      if (featureBlocks.get(index) === 'public-concourse') tower.profile = 'straight';
      ...
      const concourseAt = ids.indexOf('public-concourse');
      const concourseHeight = concourseAt < 0 ? 0 : Math.min(available * 0.54, bw * 1.5);
      const sum = fractions.reduce((a, b, i) => a + (i === concourseAt ? 0 : b), 0);
      let y = top;
      ids.forEach((id, i) => {
        const hh = i === ids.length - 1 ? baseTop - y : i === concourseAt ? concourseHeight : (available - concourseHeight) * fractions[i] / sum;
    ...
    for (let i = 0; i < towers.length - 1; i += 3) {
      ...
      const concourse = [...a.modules, ...b.modules].find(m => m.id === 'public-concourse');
      if (concourse && yy < concourse.y + concourse.h && yy + ch > concourse.y) {
        yy = concourse.y + concourse.h + h * 0.018;
