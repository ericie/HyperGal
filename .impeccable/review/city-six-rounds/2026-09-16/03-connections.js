(() => {
  ...
  function compose(w, h, value) {
    ...
    const towers = widths.map((ratio, index) => {
      ...
      if (hero) {
        ...
        ids = [pick(random, ['arcade', 'balconies']), id, pick(random, ['arcade', 'dot-slots']), pick(random, ['balconies', 'long-slits'])];
      } else {
        const candidates = narrow ? ['arcade', 'split-disks', 'bowls', 'pleats', 'dot-slots', 'long-slits'] : ['arcade', 'arcade', 'balconies', 'long-slits', 'dot-slots', 'lantern-neck', 'teeth'];
    ...
    for (let i = 0; i < towers.length - 1; i += 3) {
      ...
      const bridgeW = a.w * 0.46 + b.w * 0.68;
      const bridgeH = Math.min(h * 0.085, bridgeW * 0.48);
      ...
      connectors.push({ id: pick(random, ['bridge','bridge','hanging-bowl','cross-stair']), x: a.x + a.w * 0.54, y: yy, w: bridgeW, h: bridgeH, ink: BLACK, paper: WHITE, variant: i });
    }
  ...
  function towerOutline(tower) {
    const { x, w, top, baseTop, profile, lean } = tower;
    const hh = baseTop - top;
    ctx.beginPath();
    if (profile === 'curve') {
      ctx.moveTo(x + w * 0.12, baseTop);
      ctx.bezierCurveTo(x - w * 0.25, top + hh * 0.6, x + w * 0.42, top + hh * 0.3, x + w * 0.08, top);
      ctx.lineTo(x + w * 0.82, top);
      ctx.bezierCurveTo(x + w * 1.42, top + hh * 0.3, x + w * 0.74, top + hh * 0.65, x + w, baseTop);
    } else if (profile === 'terrace') {
      const ys = [0, 0.22, 0.53, 0.78, 1];
      const left = [0.22, 0.05, -0.13, 0.1, 0];
      const right = [0.83, 1.04, 0.88, 1.13, 1];
      ctx.moveTo(x + left[0] * w + lean, top);
      for (let i = 1; i < ys.length; i++) {
        ctx.lineTo(x + left[i - 1] * w + lean * (1 - ys[i - 1]), top + ys[i] * hh);
        ctx.lineTo(x + left[i] * w + lean * (1 - ys[i]), top + ys[i] * hh);
      }
      for (let i = ys.length - 1; i >= 0; i--) {
        ctx.lineTo(x + right[i] * w + lean * (1 - ys[i]), top + ys[i] * hh);
        if (i) ctx.lineTo(x + right[i] * w + lean * (1 - ys[i]), top + ys[i - 1] * hh);
      }
    } else ctx.rect(x, top, w, hh);
    ctx.closePath();
  }
